	  ...
	  <table class="table table-borderless">
	    ...
	    <tr>
	      <td style="text-align:right;padding:10px 0px">
	        <input type="button" value="글올리기" onclick="location.href='qaInput?qnaSw=q';" class="btn btn-secondary"/>
	      </td>
	    </tr>
	  </table>
		    ...
		    <tr>
		      ...
		      <td style="text-align:left;">
		        ...
		        <c:if test="${vo.pwd != '' && tempSw != 1}">~~
			        <c:if test="${vo.qnaSw == 'a' && tempPreQnaIdx == tempPreQnaIdx}">  <!--  && (vo.qnaSw=='a' && tempPreQnaIdx==tempQnaIdx)}"> -->  <!-- 비밀글이면서 답글이라면 원본글의 qnaIdx의 앞의 값이 같으면 답글의 qnaIdx앞의값과 같다면 글을 볼수 있게 처리한다. -->
			          <a href="qnaContent?pag=${pageVO.pag}&idx=${vo.idx}&title=${vo.title}">${vo.title}</a>
			          <c:set var="tempSw" value="1"/>
			        </c:if>
