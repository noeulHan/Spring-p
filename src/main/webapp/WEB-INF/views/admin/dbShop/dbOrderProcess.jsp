...
<div class="container" style="font-family: 'Noto Sans KR', sans-serif;">
  <div style="font-family: 'Noto Sans KR', sans-serif; font-size: 15pt;">주문/배송 확인</div>
  <hr/>
    ...
    <c:forEach var="vo" items="${vos}">
      <tr>
        <td style="text-align:center;">
          ...
          <p><input type="button" class="btn btn-outline-secondary" value="배송지정보" onclick="nWin('${vo.orderIdx}')"></p>
        </td>
        <td style="text-align:center;"><br/><img src="${ctp}/data/dbShop/product/${vo.thumbImg}" class="thumb" width="100px"/></td>
        <td align="left">
	        <p>모델명 : <span style="color:#c2935f;;font-weight:bold;">${vo.productName}</span><br/> &nbsp; &nbsp; <fmt:formatNumber value="${vo.mainPrice}"/>원</p>
		...
		<ul class="pagination justify-content-center">
			<c:if test="${pageVo.totPage != 0}">
			  ...
			  <c:if test="${pageVo.curBlock > 0}">
			    <li class="page-item"><a href="${ctp}/dbShop/dbOrderProcess?pag=${(pageVo.curBlock-1)*pageVo.blockSize + 1}" title="이전블록" class="page-link text-secondary">◀</a></li>
			  </c:if>
			  ...
			  <c:if test="${pageVo.curBlock < pageVo.lastBlock}">
			    <li class="page-item"><a href="${ctp}/dbShop/dbOrderProcess?pag=${(pageVo.curBlock+1)*pageVo.blockSize + 1}" title="다음블록" class="page-link text-secondary">▶</a>
			  </c:if>
			  <c:if test="${pageVo.pag != pageVo.totPage}">
			    <li class="page-item"><a href="${ctp}/dbShop/dbOrderProcess?pag=${pageVo.totPage}" title="마지막페이지" class="page-link" style="color:#555">▷▷</a>
			  </c:if>
			</c:if>
		</ul>
	</div>
